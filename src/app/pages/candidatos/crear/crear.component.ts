...
export class CrearComponent implements OnInit {
  ...
  constructor(private miCandidatoService: CandidatoService,
    ...
    getCandidato(id:string){
      this.miCandidatoService.getCandidato(id).subscribe(
        data=>{
          this.candidato = data;    
        }
      )
    ...
    validarDatos():boolean{
      ...
      if (this.candidato.cedula == '' || this.candidato.nombre==''|| this.candidato.apellido=='' || this.candidato.nombre_resolucion==''){
        return false;
      }
      else{
        return true;
    ...
    agregar():void{
      if(this.validarDatos()){
        this.intentoEnvio = true;
        this.miCandidatoService.agregar(this.candidato).subscribe(
          data =>{
            Swal.fire({
    ...
    editar():void{
      if(this.validarDatos()){
        this.miCandidatoService.editar(this.candidato._id,this.candidato).subscribe(
          data =>{
            Swal.fire({
              title: 'Editando Candidato',
              text:'El candidato se actualizó de manera correcta',
              icon:'success'
            });
            this.router.navigate(['pages/candidatos/listar'])
          }
